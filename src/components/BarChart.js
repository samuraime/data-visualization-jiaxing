...
export default class BarChart extends Component {
  ...
  render() {
    const defaultBox = { width: 420, height: 210, top: 0, right: 0, bottom: 20, left: 0 };
    ...
    return (
      <svg style={style} width="100%" height="100%" viewBox={`0 0 ${width + box.left + box.right} ${height + box.top + box.bottom}`}>
        <defs>
        ...
        <g transform={`translate(0,${box.bottom / 2})`}>
          {data.map((d, i) => (
            <g key={i} className={s.item} transform={`translate(0,${yStep * i})`}>
              ...
              <text x="18" y="0" style={{ alignmentBaseline: 'central', textAnchor: 'start', fontSize: 12 }}>{d.name}</text>
            </g>
          ))}
        </g>
      </svg>
